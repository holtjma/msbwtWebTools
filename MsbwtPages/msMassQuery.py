...
This file contains the contents for the "msMassQuery" utilities.  This tool searches a single dataset for many different
...
def GET():
    ...
    panel.br()
    panel.add('<textarea id="input-textbox" value="" style="height:200px; width:90%; overflow:scroll;font-family:monospace;"></textarea>')
    panel.br()
    ...
    panel.td(element.input(id="column-textbox", type="text", name="column-textbox", value="1"))
    panel.tr.close()
    ...
    panel.ol()
    panel.li('Select a dataset to query.')
    ...
    panel.li('Column: the column with the <i>k</i>-mer queries')
    panel.li('Reverse-Complement counts: check this if both forward and reverse-complement counts are required')
    panel.ul.close()
    ...
    panel.add("<script src='./static/js/massQuery.js'></script>")
